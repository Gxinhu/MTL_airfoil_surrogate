...
class AbsWeighting(nn.Module):
        ...
        if not rep_grad:
            ...
            for tn in range(self.task_num):
                if mode == "backward":
                    ...
                    raise ValueError(
                        "No support {} mode for gradient computation"
                    )
                self.zero_grad_share_params()
        else:
            if not isinstance(self.rep, dict):
                grads = torch.zeros(self.task_num, *self.rep.size()).to(
                    self.device
                )
            else:
                grads = [
                    torch.zeros(*self.rep[task].size())
    ...
    def _get_grads(self, losses, mode="backward"):
        ...
        if self.rep_grad:
            ...
            if not isinstance(self.rep, dict):
                grads = per_grads.reshape(
                    self.task_num, self.rep.size()[0], -1
                ).sum(1)
            else:
                try:
                    grads = (
                        torch.stack(per_grads).sum(1).view(self.task_num, -1)
                    )
                except:
                    raise ValueError(
    ...
    def _backward_new_grads(self, batch_weight, per_grads=None, grads=None):
        ...
        if self.rep_grad:
            ...
            new_grads = sum(
                [batch_weight[i] * grads[i] for i in range(self.task_num)]
            )
